...
(function (root, factory) {
  ...
  var COMPRESSION_EXPONENT = 2;   /* retired; kept for the export */
  ...
  /* ── THE CLAMP ───────────────────────────────────────────────────────
     FSRS decides the interval. The exam only says where it may not land.

     The previous version squashed every interval into the runway before the
     sweep week — a logarithmic map from the raw interval onto "half the days
     left" — and with an exam three weeks out that put a card at stability 16
     on day 5 and a card at stability 4.7 on day 5 too: every button on every
     card read the same number, and the rating meant nothing. Stacking extra
     reviews on a card she already knows takes time from the cards she does
     not; FSRS already spaces by memory strength, so it is left alone.

     The one rule: the next review lands on or before the exam date.

       interval = min(fsrs_interval, days_until_exam)
       days_until_exam <= 1  ->  1   (still review it)

     A sub-day learning step is not a schedule and is never touched, so a
     card rated Again still comes back this sitting. */

  /* rawMs      what FSRS asked for, for THIS rating
     maxRawMs   accepted for compatibility with every caller; unused.
  ...
  function compressToRunway(rawMs, maxRawMs, exam, now, settings) {
    ...
    var daysLeft = Math.max(1, daysBetween(exam.date, startOfDay(now)));
    if (rawMs / DAY <= daysLeft) return out;     /* already lands in time */
    out.ms = daysLeft * DAY;
    out.compressed = true;
    out.reason = 'exam';
    return out;
  ...
  function buildQueue(sched, items, units, exams, now, opts) {
    ...
    /* due THROUGH THE END OF TODAY: a review card's due_date is a day, and a
       card due today is due whether or not its timestamp has ticked past */
    var endOfToday = startOfDay(now) + DAY;
    var dueAll = items.filter(function (it) {
      return it.due_date != null && it.due_date < endOfToday;
    });
