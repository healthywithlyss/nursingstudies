...
(function (root, factory) {
  ...
  var DEFAULT_SETTINGS = {
    ...
    dailyCeiling: 120,
    /* The sweep window, in days before the exam. Written out rather than
       referring to SWEEP_DAYS because that is declared further down and would
       still be undefined here — a test pins the two together. */
    sweepDays: 7,
  ...
  function sweepDaysFrom(settings) {
    var n = settings && settings.sweepDays;
    n = Math.round(Number(n));
    return (isFinite(n) && n >= 1 && n <= 60) ? n : SWEEP_DAYS;
  }
  ...
  function projectTerm(sched, items, units, exams, now, opts) {
    ...
    for (var day = 0; day < days; day++) {
      ...
      for (var i = 0; i < sim.length; i++) {
        ...
        if (isPassed) {
          if (passedUsed >= passedBudget) { s.due = day + 1; continue; }
          passedUsed++;
          row.passedUnit++;
        } else {
          row.currentUnit++;
        ...
        var passedIt = pseudoRandom(day, i) < retention;
        if (!passedIt) {
          s.due = day;                      /* back inside the same day */
          s.stability = Math.max(0.5, s.stability * 0.4);
          /* counted again when it comes round; approximate that as one extra */
          row.total += 0;
        } else if (s.kind === 'quiz') {
          s.retiredQuiz = true;             /* a correct quiz answer retires it */
        } else {
          s.stability = s.stability * 2.2 + 1;
          var ivl = Math.max(1, Math.round(sched.intervalFor(s.stability)));
          var exam = nextExamFor(s.unit, exams, when);
          if (exam) {
            var c = compressToRunway(ivl * DAY, ivl * DAY, exam, when);
      ...
      row.total = row.newItems + row.currentUnit + row.passedUnit;
      out.push(row);
    }

    /* sweep days carry their own requirement on top */
    EXAMS.forEach(function (key) {
      ...
      if (!scope) return;
      var perDay = Math.ceil(scope / SWEEP_DAYS);
      out.forEach(function (row) {
        var d = daysBetween(date, startOfDay(row.ms));
        if (d > 0 && d <= SWEEP_DAYS) { row.sweep += perDay; row.total += perDay; }
      });
    });
  ...
  function sweepPlan(sched, items, units, exams, now, settings) {
    var today = startOfDay(now);
    var SWEEP = sweepDaysFrom(settings);
    ...
    EXAMS.forEach(function (key) {
      ...
      if (remaining > SWEEP || remaining < 0) return;
    ...
    var windows = open.map(function (w) {
      ...
      return {
        ...
        day: Math.min(SWEEP, SWEEP - w.daysRemaining + 1),
        totalDays: SWEEP,
