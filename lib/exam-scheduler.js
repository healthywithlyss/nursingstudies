...
(function (root, factory) {
  ...
  function compressToRunway(rawMs, maxRawMs, exam, now) {
    ...
    /* Inside the sweep week there is no runway left. Everything is being seen
       anyway, so a card goes to tomorrow rather than out of reach. */
    if (usable <= 0) {
      var left = Math.max(1, daysBetween(exam.date, startOfDay(now)) - 1);
      var capped = Math.min(Math.round(rawMs / DAY), left);
      out.ms = Math.max(1, capped) * DAY;
  ...
  function sweepPlan(sched, items, units, exams, now) {
    ...
    EXAMS.forEach(function (key) {
      ...
      if (best && best.date <= date) return;
      best = { key: key, date: date, daysRemaining: remaining };
    });
    if (!best) return { active: false };

    var start = best.date - SWEEP_DAYS * DAY;
    var scope = items.filter(function (it) {
      return examsCovering(unitOf(it, units)).indexOf(best.key) > -1;
    });
    var covered = [], remaining = [];
    scope.forEach(function (it) {
      if (it.last_reviewed_at != null && startOfDay(it.last_reviewed_at) >= start) covered.push(it);
      else remaining.push(it);
    });

    /* never seen, then currently failing, then everything else */
    ...
    remaining.sort(function (a, b) {
      var d = rank(a) - rank(b);
      if (d) return d;
      /* within a tier, weakest recall first */
      return predictR(sched, a, today) - predictR(sched, b, today);
    });

    /* Days still available to cover it in, the exam day itself excluded. */
    var daysLeft = Math.max(1, best.daysRemaining);
    var perDay = Math.ceil(remaining.length / daysLeft);
    return {
      active: true,
      key: best.key,
      label: best.key === 'final' ? 'Final' : (best.key === 'unit1' ? 'Unit 1' : 'Unit 2'),
      date: exams[best.key],
      daysRemaining: best.daysRemaining,
      /* day 1 of 7 on the day the window opens */
      day: Math.min(SWEEP_DAYS, SWEEP_DAYS - best.daysRemaining + 1),
      totalDays: SWEEP_DAYS,
      total: scope.length,
      covered: covered.length,
      remaining: remaining.length,
      perDay: perDay,
      neverSeen: remaining.filter(neverSeen).length,
      failing: remaining.filter(function (it) { return isFailing(sched, it, now); }).length,
      todayList: remaining.slice(0, perDay)
    };
  }
