...
(function (root, factory) {
  ...
  var DEFAULT_SETTINGS = {
    ...
    dailyCeiling: 120,
    /* Sweep windows, in days before the exam. Written out rather than
       referring to the constants, which are declared further down and would
       still be undefined here — a test pins them together. */
    sweepDaysUnit: 15,
    sweepDaysFinal: 30,
    /* Passed-unit maintenance. 20 rather than something smaller because a unit
       is roughly 600 flashcards across five lectures, and at 10 a day each card
       would be seen about 1.5 times in the 90 days between one unit test and
       the next — decay, not maintenance. */
    passedUnitCardsPerDay: 20
  };
  ...
  /* The sweep: the last stretch before an exam, where every item in the unit is
     ...
     Seven days is the default because that is what was asked for, but at real
     volume it does not survive contact: 600 flashcards in seven days is 86 a
     day on top of everything else, and the final's sweep covers both units, so
     1,200 cards is 172 a day. The window is therefore a setting, and
     suggestSweepDays works out what it would have to be to be followable. */
  var SWEEP_DAYS = 7;

  /* What the windows actually default to. A unit is roughly 600 flashcards and
     the final covers both, so seven days would be 86 and 172 a day respectively
     — numbers that do not get done. These bring both to about 40. */
  var SWEEP_DAYS_UNIT = 15;
  var SWEEP_DAYS_FINAL = 30;

  /* Items a day the sweep can add before it stops being something a person
     actually does alongside clinical. Roughly seven minutes at the measured
     pace. */
  var SUSTAINABLE_SWEEP_PER_DAY = 40;

  /* The window for ONE exam. The final gets its own because its scope is both
     units, so the same number of days is twice the daily rate. */
  function sweepDaysFrom(settings, examKey) {
    var isFinal = examKey === 'final';
    var n = settings && (isFinal ? settings.sweepDaysFinal : settings.sweepDaysUnit);
    n = Math.round(Number(n));
    if (isFinite(n) && n >= 1 && n <= 90) return n;
    /* a single sweepDays still works, for anyone who set one */
    var one = settings && settings.sweepDays;
    one = Math.round(Number(one));
    if (isFinite(one) && one >= 1 && one <= 90) return one;
    return isFinal ? SWEEP_DAYS_FINAL : SWEEP_DAYS_UNIT;
  }

  /* The widest window in play, for anything that needs one number — how far
     ahead to look for an open sweep, and how many days new material has before
     the sweeps take over. */
  function maxSweepDays(settings) {
    return Math.max(sweepDaysFrom(settings, 'unit1'), sweepDaysFrom(settings, 'final'));
  }

  /* How wide the window would have to be for `count` items to fit at a
     followable rate — the honest answer to "is this sweep survivable". */
  function suggestSweepDays(count, perDay) {
    var rate = perDay || SUSTAINABLE_SWEEP_PER_DAY;
    return Math.max(SWEEP_DAYS, Math.min(45, Math.ceil(count / rate)));
  }
  ...
  /* ══ compressing a schedule into the runway ════════════════════════════
     ...
     keeping its shape.
       ...
       squash   a logarithmic map from the raw interval onto the runway — see
                squashDays below for why it must depend on the raw interval
                alone and not on the card's own ladder.

     With 35 days to the test, FSRS asking 64/86/135 days becomes roughly
     15/16/17 — inside the runway, still in order, and a weaker card asking for
     6 days lands well before all of them.
  ...
  var COMPRESSION_EXPONENT = 2;   /* retired: see squashDays; kept for the export */

  /* How a pass splits across the ladder, for projection only. Roughly what a
     working deck looks like: mostly Good, a quarter Hard, a tenth Easy. */
  var RATING_MIX = { hard: 0.25, good: 0.65, easy: 0.10 };

  /* Days of real scheduling space before the sweep takes over. The sweep is
     wider for the final than for a unit test, so the runway it eats into is
     the window for THIS exam, not a single global number. */
  function runwayDays(exam, now, settings) {
    if (!exam) return null;
    var today = startOfDay(now);
    return daysBetween(exam.date, today) - sweepDaysFrom(settings, exam.key);
  }

  /* ── THE SQUASH ───────────────────────────────────────────────────────
     Maps a raw FSRS interval (days) onto the space actually available, as a
     function of the raw interval ALONE.

     That last clause is the fix. The previous version scaled each card against
     its own Easy interval — days = longest * (raw / maxRaw)^2 — which kept the
     four buttons of ONE card in order but divided out the card's absolute
     strength: the Good/Easy ratio is much the same on every card, so a card at
     stability 2 and a card at stability 150 both landed on day 2. For the ten
     days before a sweep opened, the scheduler stopped distinguishing what she
     knew from what she did not, which is precisely when that distinction
     matters most.

     A function of the raw interval alone is monotone across cards by
     construction: a weaker card asks for less and gets less. And because the
     four ratings on one card are already ordered in raw terms, the same
     monotonicity keeps them ordered too. Both properties, one mechanism.

       knee   below this, nothing happens — FSRS already fits with room to spare
       cap    the last useful day; the squash approaches it and never passes it
       REF    the raw interval at which the cap is reached. Two years, beyond
              anything FSRS asks for on a term's material, so nothing saturates
              early and long intervals still tell apart.

     It is logarithmic above the knee, which is what spreads 6d / 15d / 2mo /
     5mo / 1.3y across a ten-day runway as 5 / 6 / 7 / 8 / 10 instead of
     2 / 2 / 2 / 2 / 2.

     Returns real days, not whole ones. A review interval is stored as a whole
     number of days for display, but the due time itself is a timestamp, and
     keeping the fraction is what lets two ratings that round to the same day
     label still land at different moments — and still be ordered. */
  var SQUASH_REF_DAYS = 730;

  function squashDays(rawDays, knee, cap) {
    if (rawDays <= knee) return rawDays;
    var t = Math.log(1 + (rawDays - knee) / knee)
          / Math.log(1 + (SQUASH_REF_DAYS - knee) / knee);
    return knee + (cap - knee) * Math.min(1, t);
  }

  /* rawMs      what FSRS asked for, for THIS rating
     maxRawMs   accepted for compatibility with every caller, and no longer
                used: scaling against the card's own top rung is the bug this
                replaces. Both orderings come from monotonicity now.
     Returns {ms, compressed, rawMs, reason}. */
  function compressToRunway(rawMs, maxRawMs, exam, now, settings) {
    ...
    var usable = runwayDays(exam, now, settings);
    var rawDays = rawMs / DAY;

    /* Inside the sweep window there is no runway left before the exam, but
       there are still days, and the same squash applies to them: monotone in
       the raw interval, so a weak card still comes back before a strong one
       even here, where the space is shortest and a collapse is likeliest. */
    if (usable <= 0) {
      var left = Math.max(1, daysBetween(exam.date, startOfDay(now)) - 1);
      var dS = squashDays(rawDays, Math.max(1, left / 2), left);
      out.ms = Math.round(Math.min(Math.max(1, dS), left) * DAY);
      out.compressed = out.ms !== rawMs;
      out.reason = 'sweep';
      return out;
    }

    var knee = Math.max(1, usable / 2);

    /* already fits, with room to spare — leave FSRS alone */
    if (rawDays <= knee) return out;

    var d = squashDays(rawDays, knee, usable);
    out.ms = Math.round(Math.min(Math.max(1, d), usable) * DAY);
    out.compressed = true;
    out.reason = 'runway';
    return out;
  }
  ...
  /* ══ quiz questions retire ═════════════════════════════════════════════
     A quiz question asks whether she knows something. Answered correctly, it
     has done its job. Keeping 1,500 of them in a review queue treats an
     assessment as a study tool and buries the flashcards, which are the thing
     actually built for repetition.

     A missed question stays until it is answered right, and separately becomes
     a flashcard, because a miss is a knowledge gap and gaps are what cards are
     for. */
  function isRetired(item) {
    return item.kind === 'quiz' && !!item.retired_at;
  }

  /* Everything the scheduler should still be looking at. */
  function liveItems(items) {
    return items.filter(function (it) { return !isRetired(it); });
  }

  /* ══ passed-unit maintenance ═══════════════════════════════════════════
     After a unit's test, its material still has to survive to the final, but it
     should not crowd out the unit being studied now. A separate daily ceiling
     governs it — a ceiling, not a quota, so a quiet day is simply shorter.

     Two things are exempt, and both matter:
       - anything still in learning or relearning, which is mid-repair
       - anything she was FAILING when that unit's exam came round, which is
         exactly the material the maintenance pass exists for */
  function isPassedUnit(item, units, exams, now) {
    var unit = unitOf(item, units);
    var own = parseDate(unit === 2 ? exams.unit2 : exams.unit1);
    return own !== null && own < startOfDay(now);
  }

  /* `failingAtExam` is a set of "kind:id" the caller worked out from the
     attempt history — the last attempt on or before that unit's exam was a
     miss. It is passed in rather than derived here because it needs the
     attempts table, which this file has no business knowing about. */
  function passedUnitExempt(item, failingAtExam) {
    if (item.state === 'learning' || item.state === 'relearning') return true;
    if (!failingAtExam) return false;
    return !!failingAtExam[item.kind + ':' + item.id];
  }

  /* ══ the term projection ═══════════════════════════════════════════════
     What every day between now and the final actually costs, split the way the
     work feels: new material, the unit being studied now, and the units already
     examined. Simulated forward rather than estimated, because the question is
     whether it is survivable alongside clinical, and an average would hide the
     walls.

     The simulation is deliberately simple and states its assumptions:
       - a new item introduced today comes back on FSRS's first-step ladder and
         then at roughly its scheduled interval
       - reviews are assumed passed at `assumeRetention`, since a projection
         that assumes she fails everything is a worst case, not a forecast
       - a missed review returns inside the same day and is counted again
       - quiz questions retire on a pass, so they drain rather than accumulate
       - sweep days add the sweep's own requirement on top */
  function projectTerm(sched, items, units, exams, now, opts) {
    opts = opts || {};
    var st = Object.assign({}, DEFAULT_SETTINGS, opts.settings || {});
    var days = opts.days || 120;
    var retention = opts.assumeRetention == null ? 0.85 : opts.assumeRetention;
    var today = startOfDay(now);
    var live = liveItems(items);

    /* Auto-generated cards. A missed quiz question spawns a flashcard from its
       fact_tested, so the card bank GROWS as she works — the projection has to
       carry that or it understates the back half of the term.

       missRate    how often a quiz question is missed. Defaults to 1-retention,
                   but the real figure from her own attempts should be passed in.
       acceptRate  1 answers "what if I accept every suggestion?" honestly.
       dedupRate   the share suppressed as near-duplicates. Measured at 0 across
                   31,695 same-objective pairs in the real bank, so it defaults
                   to 0 rather than flattering the numbers.
       Set enabled:false to see the old, card-bank-is-fixed projection. */
    var autoOpt = opts.autoCards || {};
    var autoOn = autoOpt.enabled !== false;
    var acceptRate = autoOpt.acceptRate == null ? 1 : autoOpt.acceptRate;
    var dedupRate = autoOpt.dedupRate == null ? 0 : autoOpt.dedupRate;
    var quizMiss = autoOpt.missRate == null ? (1 - retention) : autoOpt.missRate;
    var autoBorn = [];                 /* {unit, day} for every card generated */

    /* Days she does no work at all. Nothing is reviewed, no new material is
       introduced, and everything due rolls forward — which is the only honest
       way to answer "what if I skip three days?". */
    var skip = {};
    (opts.skip || []).forEach(function (d) { skip[d] = true; });

    /* a light mutable copy: due day, kind, unit, whether it is still unseen */
    var sim = live.map(function (it) {
      var d = it.due_date == null ? 0 : Math.max(0, daysBetween(it.due_date, today));
      return {
        kind: it.kind, unit: unitOf(it, units),
        unseen: neverSeen(it),
        due: neverSeen(it) ? null : d,
        stability: it.stability || 1,
        retiredQuiz: false
      };
    });

    /* Projected load per day, so the simulation levels the way the app does.
       Without it the projection piled 648 reviews onto exam eve: the clamp
       compresses every ladder into the same runway, and across a thousand items
       those compressed intervals synchronise near the deadline. The app slides
       each item onto the lightest nearby day; the forecast has to as well or it
       reports a peak that will not happen. */
    var load = [];
    var addLoad = function (d) { load[d] = (load[d] || 0) + 1; };
    sim.forEach(function (x) { if (!x.unseen && x.due != null) addLoad(x.due); });

    /* the lightest day within `room` of `want`, never before tomorrow and never
       past the last useful day before the exam */
    function levelled(want, room, from, latestDay) {
      if (room <= 0) return want;
      var best = want, bestLoad = load[want] || 0;
      for (var d = want - room; d <= want + room; d++) {
        if (d <= from || d > latestDay) continue;
        var l = load[d] || 0;
    ...
    /* The sweep, worked out up front, because it sets the day's budget rather
       than sitting on top of it. In buildQueue the sweep list is the HEAD of
       the queue and the cap becomes max(ceiling, sweep size) — so a sweep day
       is a bigger day, not a normal day plus a sweep. The projection said
       "normal day plus a sweep" and reported totals the app would never ask
       for. Auto-generated cards widen the scope; they are folded in below once
       the run knows how many there were. */
    var sweepAdd = [];
    var sweepScope = {};
    EXAMS.forEach(function (key) {
      var date = parseDate(exams[key]);
      if (date === null) return;
      var scope = live.filter(function (it) {
        return examsCovering(unitOf(it, units)).indexOf(key) > -1
          && (it.kind === 'card' || neverSeen(it) || it.state === 'relearning');
      }).length;
      if (!scope) return;
      var win = sweepDaysFrom(st, key);
      var workDays = 0;
      for (var d0 = 0; d0 < days; d0++) {
        var dd = daysBetween(date, today + d0 * DAY);
        if (dd > 0 && dd <= win && !skip[d0]) workDays++;
      }
      if (!workDays) workDays = win;
      sweepScope[key] = { date: date, win: win, scope: scope, workDays: workDays };
      var per = Math.ceil(scope / workDays);
      for (var d1 = 0; d1 < days; d1++) {
        var d2 = daysBetween(date, today + d1 * DAY);
        if (d2 > 0 && d2 <= win && !skip[d1]) sweepAdd[d1] = (sweepAdd[d1] || 0) + per;
      }
    });

    var out = [];
    for (var day = 0; day < days; day++) {
      var when = today + day * DAY;
      var row = { day: day, ms: when, date: new Date(when),
                  newItems: 0, currentUnit: 0, passedUnit: 0, sweep: 0, total: 0,
                  autoCards: 0, autoBacklog: 0, extra: 0,
                  demand: 0, deferred: 0, backlog: 0, held: 0 };
      var spawn = [];
      row.sweep = sweepAdd[day] || 0;
      /* what the app would actually put in front of her today */
      var budget = Math.max(st.dailyCeiling, row.sweep);
      var done = 0;

      if (skip[day]) {
        row.skipped = true;
        row.sweep = 0;
        row.demand = sim.reduce(function (a, x) {
          return a + (!x.retiredQuiz && !x.unseen && x.due != null && x.due <= day ? 1 : 0);
        }, 0);
        row.deferred = row.demand;
        for (var sk = 0; sk < sim.length; sk++) {
          if (sim[sk].due === day) sim[sk].due = day + 1;
        }
        row.autoBacklog = sim.reduce(function (a, x) {
          return a + (x.auto && x.unseen ? 1 : 0);
        }, 0);
        row.backlog = row.demand;
        out.push(row);
        continue;
      }

      /* which units are already examined on this simulated day */
      var passed = {};
      [1, 2].forEach(function (u) {
        var own = parseDate(u === 2 ? exams.unit2 : exams.unit1);
        passed[u] = own !== null && own < when;
      });

      /* new material, capped per kind */
      var introduced = { card: 0, quiz: 0 };
      var capFor = { card: st.newCardsPerDay, quiz: st.newQuizPerDay };
      var passedBudget = st.passedUnitCardsPerDay;
      var passedUsed = 0;

      for (var i = 0; i < sim.length; i++) {
        var s = sim[i];
        if (s.retiredQuiz) continue;

        if (s.unseen) {
          if (introduced[s.kind] >= capFor[s.kind]) continue;
          if (done >= budget) continue;      /* the ceiling holds for new material */
          done++;
          introduced[s.kind]++;
          s.unseen = false;
          row.newItems++;
          /* it comes back tomorrow-ish while it is in learning */
          s.due = day + 1;
          addLoad(s.due);
          continue;
        }

        /* `>` not `!==`: anything whose day has passed is overdue, not gone.
           An exact-match test stranded every missed item permanently, because a
           miss set its due day to TODAY and today's pass had already gone by it. */
        if (s.due > day) continue;

        row.demand++;
        var isPassed = passed[s.unit];
        /* Material from an examined unit is throttled to its own allowance BY
           DESIGN — it is maintenance, not a backlog, and counting it as one
           made a healthy schedule look like it was drowning. Reported
           separately as `held`. */
        if (isPassed && passedUsed >= passedBudget) {
          s.due = day + 1; row.held++; continue;
        }
        /* Over the ceiling the item rolls to tomorrow. This is the number that
           answers "does the backlog level out or wall up" — it is only a real
           pile if `deferred` keeps growing rather than draining. */
        if (done >= budget) { s.due = day + 1; row.deferred++; continue; }
        done++;
        if (isPassed) { passedUsed++; row.passedUnit++; } else { row.currentUnit++; }

        /* did she get it right? Quiz questions carry their own miss rate,
           because that rate is also what decides how many cards get generated. */
        var passChance = s.kind === 'quiz' ? (1 - quizMiss) : retention;
        var passedIt = pseudoRandom(day, i) < passChance;
        if (!passedIt) {
          /* It comes back inside the same session — that is the whole point of
             the learning steps — so it costs a second look today, and it is due
             again tomorrow rather than disappearing. */
          row.extra++;
          done++;
          s.stability = Math.max(0.5, s.stability * 0.4);
          s.due = day + 1;
          addLoad(s.due);
          /* the first miss is the one that offers a card; a question already
             turned into a card, or already rejected, does not ask twice */
          if (autoOn && s.kind === 'quiz' && !s.spawned) {
            s.spawned = true;
            if (pseudoRandom(day + 7919, i) < acceptRate * (1 - dedupRate)) {
              spawn.push({ kind: 'card', unit: s.unit, unseen: true, due: null,
                           stability: 1, retiredQuiz: false, auto: true });
              autoBorn.push({ unit: s.unit, day: day });
              row.autoCards++;
            }
          }
        } else if (s.kind === 'quiz') {
          s.retiredQuiz = true;             /* a correct quiz answer retires it */
        } else {
          /* A pass is not one rating. Modelling every pass as the same rating
             was wrong in a way that mattered: with rawMs === maxRawMs the
             compression ratio is always 1, so every item passed on a given day
             got the SAME interval and the whole bank marched in lockstep into
             exam eve. Spreading the ratings is what actually happens, and it
             desynchronises the cohorts. */
          var roll = pseudoRandom(day + 3571, i);
          var grow = roll < RATING_MIX.hard ? 1.3
            : (roll < RATING_MIX.hard + RATING_MIX.good ? 2.2 : 3.5);
          s.stability = s.stability * grow + 1;
          var ivl = Math.max(1, Math.round(sched.intervalFor(s.stability)));
          /* the top of the ladder for THIS card, which is what the compression
             scales against — the same reference the app uses */
          var topIvl = Math.max(ivl,
            Math.round(sched.intervalFor(s.stability / grow * 3.5 + 1)));
          var exam = nextExamFor(s.unit, exams, when);
          var latest = Infinity;
          if (exam) {
            var c = compressToRunway(ivl * DAY, topIvl * DAY, exam, when, st);
            ivl = Math.max(1, Math.round(c.ms / DAY));
            latest = daysBetween(exam.date - EXAM_BUFFER_DAYS * DAY, today);
          }
          s.due = levelled(day + ivl, slideRoom(ivl), day, latest);
          addLoad(s.due);
        }
      }

      /* generated cards join the queue tomorrow, behind the same new-items cap
         as everything else — which is exactly why they lengthen the runway
         rather than spiking any single day */
      if (spawn.length) sim = sim.concat(spawn);
      row.autoBacklog = sim.reduce(function (a, x) {
        return a + (x.auto && x.unseen ? 1 : 0);
      }, 0);

      row.total = row.newItems + row.currentUnit + row.passedUnit + row.extra;
      row.demand += row.newItems;
      /* Everything due tomorrow that is NOT passed-unit maintenance. Against a
         ceiling of ~120 this is the number that says whether a gap drains or
         piles up; maintenance is throttled deliberately and is counted apart. */
      row.backlog = sim.reduce(function (a, x) {
        if (x.retiredQuiz || x.unseen || x.due == null || x.due > day + 1) return a;
        return a + (passed[x.unit] ? 0 : 1);
      }, 0);
      row.maintenance = sim.reduce(function (a, x) {
        if (x.retiredQuiz || x.unseen || x.due == null || x.due > day + 1) return a;
        return a + (passed[x.unit] ? 1 : 0);
      }, 0);
      out.push(row);
    }

    /* Auto-generated cards widen each sweep's scope. The per-day figure was
       fixed before the run knew how many there would be, so the difference is
       added here rather than pretended away. */
    Object.keys(sweepScope).forEach(function (key) {
      var g = sweepScope[key];
      var opensOn = daysBetween(g.date, today) - g.win;
      var grown = autoBorn.reduce(function (a, b) {
        return a + (b.day <= opensOn && examsCovering(b.unit).indexOf(key) > -1 ? 1 : 0);
      }, 0);
      if (!grown) return;
      var before = Math.ceil(g.scope / g.workDays);
      var after = Math.ceil((g.scope + grown) / g.workDays);
      var delta = after - before;
      if (delta <= 0) return;
      out.forEach(function (row) {
        var d = daysBetween(g.date, startOfDay(row.ms));
        if (d > 0 && d <= g.win && !row.skipped) {
          row.sweep += delta;
          if (row.total < row.sweep) row.total = row.sweep;
        }
      });
    });
  ...
  /* Deterministic, so the same inputs always give the same projection — a
     forecast that changes every time it is opened is not a forecast. */
  function pseudoRandom(a, b) {
    var x = Math.sin(a * 12.9898 + b * 78.233) * 43758.5453;
    return x - Math.floor(x);
  }

  /* Summary of a projection: the shape of the term in a few numbers. */
  function projectionSummary(rows, opts) {
    opts = opts || {};
    var minutesPer = opts.minutesPerItem == null ? 0.18 : opts.minutesPerItem;
    var totals = rows.map(function (r) { return r.total; });
    var sorted = totals.slice().sort(function (a, b) { return a - b; });
    var pct = function (p) { return sorted[Math.min(sorted.length - 1,
      Math.floor(sorted.length * p))] || 0; };
    var peak = rows.reduce(function (m, r) { return r.total > m.total ? r : m; }, rows[0] || { total: 0 });
    return {
      days: rows.length,
      median: pct(0.5),
      p90: pct(0.9),
      peak: peak.total,
      peakDate: peak.date,
      medianMinutes: Math.round(pct(0.5) * minutesPer),
      p90Minutes: Math.round(pct(0.9) * minutesPer),
      peakMinutes: Math.round(peak.total * minutesPer),
      minutesPerItem: minutesPer,
      totalItems: totals.reduce(function (a, b) { return a + b; }, 0),
      autoCards: rows.reduce(function (a, r) { return a + (r.autoCards || 0); }, 0),
      autoBacklogPeak: rows.reduce(function (m, r) {
        return Math.max(m, r.autoBacklog || 0); }, 0),
      skippedDays: rows.reduce(function (a, r) { return a + (r.skipped ? 1 : 0); }, 0)
    };
  }

  /* ── the miss rate, measured rather than guessed ───────────────────────
     The projection needs to know how often a quiz question is missed, because
     that rate decides both how much re-work there is and how many flashcards
     get generated. A guessed 20-25% is a reasonable starting point, but it is a
     guess, and it should stop being one as soon as there is data.

     Only FIRST attempts count. A question answered wrong then right three times
     is one miss and one card, not three; averaging over every attempt would
     inflate both. Below `minSample` the measured figure is too noisy to trust,
     so the fallback is used and `measured` says which she is looking at. */
  var MISS_RATE_MIN_SAMPLE = 30;
  var ASSUMED_MISS_RATE = 0.225;          /* the midpoint of the 20-25% guess */

  function missRateFrom(attempts, opts) {
    opts = opts || {};
    var min = opts.minSample == null ? MISS_RATE_MIN_SAMPLE : opts.minSample;
    var fallback = opts.fallback == null ? ASSUMED_MISS_RATE : opts.fallback;
    var first = {};
    (attempts || []).forEach(function (a) {
      var id = a.question_id;
      if (id == null) return;
      var t = typeof a.created_at === 'number' ? a.created_at : Date.parse(a.created_at);
      if (!isFinite(t)) t = 0;
      var cur = first[id];
      if (!cur || t < cur.t) first[id] = { t: t, correct: !!a.is_correct };
    });
    var ids = Object.keys(first);
    var misses = 0;
    ids.forEach(function (k) { if (!first[k].correct) misses++; });
    var measured = ids.length >= min;
    return {
      rate: measured ? misses / ids.length : fallback,
      measured: measured,
  ...
  /* ── what a gap actually costs ─────────────────────────────────────────
     A median is only true if the days happen. This runs the same projection
     twice — once clean, once with `skipDays` consecutive days missed starting
     at `from` — and answers the question that matters: does the backlog level
     out, or does it wall up?

     Recovery is the first day after the gap on which the skipped run's load is
     back within `tolerance` of the clean run for `settle` days running. If it
     never gets there inside the horizon, `recovered` is false and `recoveryDay`
     is null — that is the unrecoverable-pile failure mode, stated plainly. */
  function skipImpact(sched, items, units, exams, now, opts) {
    opts = opts || {};
    var from = opts.from == null ? 7 : opts.from;
    var n = opts.skipDays == null ? 3 : opts.skipDays;
    var tol = opts.tolerance == null ? 0.1 : opts.tolerance;
    var settle = opts.settle == null ? 3 : opts.settle;
    var skip = [];
    for (var i = 0; i < n; i++) skip.push(from + i);

    var base = projectTerm(sched, items, units, exams, now, opts);
    var gapped = projectTerm(sched, items, units, exams, now,
      Object.assign({}, opts, { skip: skip }));

    var worst = 0, worstDay = null, run = 0, recoveryDay = null;
    for (var d = from + n; d < base.length; d++) {
      var b = base[d].total, g = gapped[d].total;
      if (g - b > worst) { worst = g - b; worstDay = d; }
      var within = b === 0 ? g === 0 : (g - b) / Math.max(1, b) <= tol;
      if (within) {
        run++;
        if (run >= settle && recoveryDay === null) recoveryDay = d - settle + 1;
      } else { run = 0; }
    }

    var sum = function (rows) {
      return rows.reduce(function (a, r) { return a + r.total; }, 0); };
    return {
      skippedDays: skip.slice(),
      base: projectionSummary(base, opts),
      gapped: projectionSummary(gapped, opts),
      /* how much work the gap deferred, and where it landed */
      deferred: sum(base.slice(0, from + n)) - sum(gapped.slice(0, from + n)),
      worstExtra: worst,
      worstDay: worstDay,
      worstDate: worstDay == null ? null : gapped[worstDay].date,
      recovered: recoveryDay !== null,
      recoveryDay: recoveryDay,
      /* days from the end of the gap until the load is normal again */
      catchUpDays: recoveryDay === null ? null : recoveryDay - (from + n),
      rows: gapped
    };
  }
  ...
  /* An item counts as never seen if it has NEVER BEEN GRADED. Nothing else.

     This used to be an OR chain — no last_reviewed_at, OR no repetitions, OR
     state 'new' — and any one of the three was enough. A card mid-way through
     its learning steps whose `repetitions` column happened to read 0 was
     therefore counted as unseen by the coverage rules and served as new
     material by the queue, spending a slot of the daily new-card cap on work
     already begun.

     Deliberately the SAME rule as StudySession.isNewItem: the scheduler and the
     queue disagreeing about what "new" means is how an item gets served as new
     material by one and counted as a review by the other. A test pins the two
     together. */
  function neverSeen(item) {
    return !item.last_reviewed_at && (!item.state || item.state === 'new');
  }
  ...
  function buildQueue(sched, items, units, exams, now, opts) {
    ...
    var today = startOfDay(now);
    /* a retired question is finished with, everywhere */
    items = liveItems(items);
    var failingAtExam = opts.failingAtExam || null;
    ...
    var sweep = sweepPlan(sched, items, units, exams, now, st);
    ...
    /* Material from a unit already examined is maintenance, and it gets its own
       ceiling so it cannot crowd out the unit being studied now. Anything mid-
       repair, or anything she was failing when that unit's exam came round, is
       exempt — that is precisely what maintenance is for. */
    var passedUsed = 0, passedHeld = 0;
    review = review.filter(function (it) {
      if (!isPassedUnit(it, units, exams, now)) return true;
      if (passedUnitExempt(it, failingAtExam)) return true;
      if (passedUsed < st.passedUnitCardsPerDay) { passedUsed++; return true; }
      passedHeld++;
      return false;
    });
    ...
    return {
      ...
      passedUnit: { shown: passedUsed, held: passedHeld, cap: st.passedUnitCardsPerDay },
  ...
  function sweepPlan(sched, items, units, exams, now, settings) {
    var today = startOfDay(now);
    /* each exam has its own window now, so the constant is per-exam below */
    var SWEEP_MAX = maxSweepDays(settings);
    ...
    EXAMS.forEach(function (key) {
      ...
      var win = sweepDaysFrom(settings, key);
      if (remaining > win || remaining < 0) return;
      open.push({ key: key, date: date, daysRemaining: remaining, window: win });
    });
    ...
    /* "See every question" stops meaning anything once a correct answer retires
       one. The sweep is every FLASHCARD for the unit, plus the questions she has
       never attempted or is still getting wrong. */
    items = liveItems(items).filter(function (it) {
      if (it.kind !== 'quiz') return true;
      return neverSeen(it) || isFailing(sched, it, now)
        || it.state === 'learning' || it.state === 'relearning';
    });
    items.forEach(function (it) {
    ...
    var windows = open.map(function (w) {
      ...
      return {
        ...
        day: Math.min(w.window, w.window - w.daysRemaining + 1),
        totalDays: w.window,
  ...
  function newItemPlan(items, units, exams, now, settings) {
    ...
    EXAMS.forEach(function (key) {
      ...
      var days = Math.max(0, daysToExam - sweepDaysFrom(st, key));
      var scope = items.filter(function (it) {
        return examsCovering(unitOf(it, units)).indexOf(key) > -1;
      });
      [['card', st.newCardsPerDay, 'flashcards'], ['quiz', st.newQuizPerDay, 'quiz questions']]
        .forEach(function (row) {
          ...
          out.push({
            ...
            daysToExam: daysToExam, sweepDays: sweepDaysFrom(st, key),
            willSee: willSee,
  ...
  function buildPlan(input) {
    ...
    return {
      ...
      queue: buildQueue(sched, items, units, exams, now,
        Object.assign({}, input, { settings: st, failingAtExam: input.failingAtExam })),
      ...
      sweep: sweepPlan(sched, items, units, exams, now, st),
  ...
  return {
    ...
    missRateFrom: missRateFrom,
    MISS_RATE_MIN_SAMPLE: MISS_RATE_MIN_SAMPLE,
    ASSUMED_MISS_RATE: ASSUMED_MISS_RATE,
    isRetired: isRetired,
    liveItems: liveItems,
    isPassedUnit: isPassedUnit,
    passedUnitExempt: passedUnitExempt,
    compressToRunway: compressToRunway,
    squashDays: squashDays,
    SQUASH_REF_DAYS: SQUASH_REF_DAYS,
    ...
    slideRoom: slideRoom,
    sweepPlan: sweepPlan,
    sweepDaysFrom: sweepDaysFrom,
    maxSweepDays: maxSweepDays,
    SWEEP_DAYS_UNIT: SWEEP_DAYS_UNIT,
    SWEEP_DAYS_FINAL: SWEEP_DAYS_FINAL,
    suggestSweepDays: suggestSweepDays,
    SUSTAINABLE_SWEEP_PER_DAY: SUSTAINABLE_SWEEP_PER_DAY,
