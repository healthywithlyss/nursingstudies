/* ══════════════════════════════════════════════════════════════════════════
   ...
   then a learning card whose moment has NOT come — learning ahead beats
   watching a timer while other cards exist. serve() is next() for a screen:
   it remembers what is showing, so a rating lands on the card that was read,
   and it never hands back the card that was just rated while anything else
   is available. When that card is the only one left it waits (null) until
   its time, unless the caller asks to drill ahead.
...
(function (root, factory) {
  ...
  /* GENUINELY NEW = never rated. Nothing else.

     This used to be an OR chain — no last_reviewed_at, OR no repetitions, OR
     state 'new' — and any one of the three was enough to call an item new. A
     card mid-way through its learning steps whose `repetitions` column happened
     to read 0 was therefore served as new material AND counted against the
     daily new-card cap, which is how eight cards left in progress came back as
     eight of that day's thirty.

     A card in learning is not new. It is work already begun: it is served
     whatever the cap says, and it does not spend any of it. */
  ...
  function Session(opts) {
    ...
    this.items = {};
    this.current = null;      /* the card on screen, set by serve() */
    this.currentAnswered = false;
    this.served = [];         /* every card shown: {id, at, due, early, ahead, repeat} */
  ...
  /* What to show now. Candidates, in order of preference:
       1. a learning card whose time has come, earliest first
       2. the next scheduled card
       3. a learning card whose moment has NOT come, earliest first —
          learning a little ahead beats watching a timer while cards remain
     opts.exclude is the card that was answered last. It is never the
     immediate next card while anything else is available: a card rated Again
     and shown again two seconds later teaches nothing. When it is the only
     card left it is returned once its time has come, or when opts.ahead says
     the user chose to drill ahead; otherwise null, and waitMs() says how long. */
  Session.prototype.next = function (now, opts) {
    ...
    var due = live.filter(function (id) { return self.dueAt[id] <= t; });
    var early = live.filter(function (id) { return self.dueAt[id] > t; });
    var sched = this.order.filter(function (id) {
      return !self.doneIds[id] && live.indexOf(id) === -1;
    });
    var cands = due.concat(sched, early);
    if (!cands.length) return null;

    var ex = opts.exclude;
    if (ex == null) return this.items[cands[0]];
    var others = cands.filter(function (id) { return id !== ex; });
    if (others.length) return this.items[others[0]];
    /* the only card left is the one just answered */
    if (self.dueAt[ex] <= t || opts.ahead) return this.items[ex];
    return null;
  };

  /* next() for a screen. Remembers what is showing, so answer() can be given
     the card that was actually read rather than whichever learning card came
     due while it was being read — that mistake both mis-rates the due card
     and leaves the read card on screen, which looks exactly like "the card I
     just rated came straight back". Logs every card shown with when it was
     wanted and when it was shown, so a session can be audited afterwards.
     A re-paint of the same unanswered card is not a new serve. */
  Session.prototype.serve = function (now, opts) {
    var t = now == null ? this.now : now;
    opts = opts || {};
    var ex = this.currentAnswered ? this.current : null;
    var card = this.next(t, { exclude: ex, ahead: !!opts.ahead });
    if (!card) return null;
    if (card.id !== this.current || this.currentAnswered) {
      var early = this.dueAt[card.id] > t;
      this.served.push({
        id: card.id, at: t, due: this.dueAt[card.id], early: early,
        ahead: early && !!opts.ahead,
        repeat: ex != null && card.id === ex       /* only ever when it was the only card left */
      });
    }
  ...
  /* Local midnight of the day containing `ms`. The one place this library
     depends on the runtime's timezone: a review is due on a DAY, and the day
     is hers, not UTC's. */
  function startOfDay(ms) { var d = new Date(ms); d.setHours(0, 0, 0, 0); return d.getTime(); }

  /* Apply the clamp to one rating's raw outcome, then put a review on its DAY.

     A due date used to be the exact review time plus the interval, so a card
     reviewed at 2:45 pm was due at 2:45 pm N days later and the page counted
     down the minutes. A review card is due by day: its due is local midnight
     of (now + interval), and interval_days is the whole days between that
     day and today. Learning and relearning steps are minutes and keep their
     exact time. The FSRS memory state (stability, difficulty, lapses) is
     never touched here — only WHEN the card comes back. */
  Session.prototype.place = function (itemId, rating, now) {
    ...
    if (after.state === 'review' && c.ms >= DAY) {
      var day = startOfDay(o.t + c.ms);
      after = Object.assign({}, after, {
        due: day,
        interval_days: Math.max(1, Math.round((day - startOfDay(o.t)) / DAY))
      });
    }
    return {
  ...
  Session.prototype.answer = function (itemId, rating, now) {
    ...
    this.history.push({
      ...
      current: this.current, currentAnswered: this.currentAnswered,
      servedLen: this.served.length
    });
    this.current = itemId;
  ...
  Session.prototype.undo = function () {
    ...
    /* it goes back on screen as the unanswered card it was, and anything
       served after that answer never happened */
    this.current = h.current; this.currentAnswered = h.currentAnswered;
    if (h.servedLen != null) this.served.length = Math.min(this.served.length, h.servedLen);
  ...
  Session.prototype.toJSON = function () {
    return {
      ...
      current: this.current, currentAnswered: this.currentAnswered, served: this.served
    };
  };
  ...
  Session.restore = function (saved, items, opts) {
    ...
    s.current = saved.current != null && byId[saved.current] ? saved.current : null;
    s.currentAnswered = s.current != null && !!saved.currentAnswered;
    s.served = (saved.served || []).filter(function (e) { return e && byId[e.id]; });
    return s;
  };

  /* The day's queue over a set of items, and the daily new-card budget.

     New cards are budgeted per DAY, not per session: the limit minus the
     cards introduced today (introduced_at on or after opts.dayStart), counted
     over opts.all — the whole course — because the limit is a course setting
     and a selection of one lecture must not get a fresh 30. Learning and due
     are never capped: a card that is due is due. done is what was reviewed
     today and is now scheduled beyond today — finished for the day.

     DUE MEANS DUE TODAY, NOT DUE THIS HOUR. A review card's due_date carries
     the clock time of the review that scheduled it, so a card reviewed at
     three in the afternoon came back at three the next day and hid all
     morning. Anything whose due date falls on or before today's local date
     (before opts.dayEnd, local midnight tonight) is available from midnight.
     Learning steps are minutes, not days, and keep their exact time. */
  function todayQueue(items, opts) {
    var now = opts.now == null ? Date.now() : opts.now;
    var dayStart = opts.dayStart == null ? now - (now % DAY) : opts.dayStart;
    var dayEnd = opts.dayEnd == null ? dayStart + DAY : opts.dayEnd;
    var limit = opts.newCardsPerDay == null ? Infinity : Math.max(0, Number(opts.newCardsPerDay) || 0);
    var all = opts.all || items;
    var introducedToday = all.filter(function (i) {
      return i.introduced_at != null && i.introduced_at >= dayStart;
    }).length;
    var budget = Math.max(0, limit - introducedToday);
    var newOnes = [], learning = [], due = [], done = [];
    (items || []).forEach(function (i) {
      if (isNewItem(i)) newOnes.push(i);
      else if (i.state === 'learning' || i.state === 'relearning') learning.push(i);
      else if (i.due_date != null && i.due_date < dayEnd) due.push(i);
      else if (i.last_reviewed_at != null && i.last_reviewed_at >= dayStart) done.push(i);
    });
    due.sort(function (a, b) { return a.due_date - b.due_date; });
    return {
      newItems: newOnes.slice(0, budget),
      newAvailable: newOnes.length,
      newBudget: budget,
  ...
  /* Per-objective progress for a sidebar: how far into each lecture she is,
     what is left, and how well the started cards are known. `recall` is the
     scheduler's P(recall) right now — (stability, elapsedDays) -> 0..1 —
     averaged over started cards that have a stability. A card in two
     objectives counts in both. */
  function objectiveStats(items, opts) {
    opts = opts || {};
    var now = opts.now == null ? Date.now() : opts.now;
    var dayStart = opts.dayStart;
    /* same rule as todayQueue: due on today's date counts from midnight */
    var dayEnd = opts.dayEnd != null ? opts.dayEnd : (dayStart != null ? dayStart + DAY : now);
    var recallFn = typeof opts.recall === 'function' ? opts.recall : null;
    var out = {};
    (items || []).forEach(function (i) {
      (i.objectiveIds || []).forEach(function (o) {
        var s = out[o] || (out[o] = { total: 0, started: 0, newLeft: 0, learning: 0, due: 0,
          learned: 0, doneToday: 0, pctStarted: 0, recall: null, _rs: 0, _rn: 0 });
        s.total++;
        if (isNewItem(i)) { s.newLeft++; return; }
        s.started++;
        if (i.state === 'learning' || i.state === 'relearning') s.learning++;
        else {
          s.learned++;
          if (i.due_date != null && i.due_date < dayEnd) s.due++;
          else if (dayStart != null && i.last_reviewed_at != null && i.last_reviewed_at >= dayStart) s.doneToday++;
        }
        if (recallFn && i.stability > 0 && i.last_reviewed_at != null) {
          s._rs += recallFn(i.stability, Math.max(0, (now - i.last_reviewed_at) / DAY));
          s._rn++;
        }
      });
    });
    Object.keys(out).forEach(function (o) {
      var s = out[o];
      s.pctStarted = s.total ? Math.round(100 * s.started / s.total) : 0;
      s.recall = s._rn ? s._rs / s._rn : null;
      delete s._rs; delete s._rn;
    });
    return out;
  }

  /* The rule the served log must obey, for tests and audits: one card never
     occupies two consecutive positions unless it was the only card left AND
     it was shown on time or by an explicit choice to drill ahead. Returns the
     first offending pair, or null. */
  function repeatIn(served) {
    for (var i = 1; i < served.length; i++) {
      var a = served[i - 1], b = served[i];
      if (a.id !== b.id) continue;
      if (b.repeat && (!b.early || b.ahead)) continue;
      return { at: i, prev: a, next: b };
    }
    return null;
  }
